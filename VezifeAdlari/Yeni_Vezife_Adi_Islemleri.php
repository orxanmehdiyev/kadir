...
if (!isset($_POST['Deyer'])) {
	...
	if ($Vezife_Adlari_Ad!="") {
		$Vezife_Adlari_Sayi_Sor=$db->prepare("SELECT * FROM vezife_adlari where Vezife_Adlari_Ad=:Vezife_Adlari_Ad ");
		...
		if (!$Vezife_Adlari_Sayi_Say>0) {
			if ($Vezife_Adlari_Sira!="") {
				$Vezife_Adlari_Sayi_Sor=$db->prepare("SELECT * FROM vezife_adlari where Vezife_Adlari_Sira=:Vezife_Adlari_Sira ");
				$Vezife_Adlari_Sayi_Sor->execute(array(
					'Vezife_Adlari_Sira'=>$Vezife_Adlari_Sira));
				$Vezife_Adlari_Sayi_Say=$Vezife_Adlari_Sayi_Sor->rowCount();
				if (!$Vezife_Adlari_Sayi_Say>0) {
					$Elave_Et=$db->prepare("INSERT INTO vezife_adlari SET 
						Vezife_Adlari_Ad=:Vezife_Adlari_Ad, 
						Vezife_Adlari_Sira=:Vezife_Adlari_Sira
						");
					...
					if ($Insert) {
						$Vezife_Adlari_Id = $db->lastInsertId();
					...
					<div class="over-y genislik">
						<table class="table table-bordered">
							...
							<?php 	}	?>
						<?php 	}else{
							echo "error_2005";/*Ikinci Insert Xeta*/
							exit;
						}
					}else{
						echo "error_2004";/*Birinci Insert Xeta*/
						exit;
					}
				}else{
					echo "error_2003";/*Vezife Sira Nomresi Bazada var*/
					exit;
				}
			}else{
				echo "error_2002";/*Vezife Sira Nomresi bosdur*/
				exit;
			}

		}else{
			echo "error_2001";/*Vezife adı bazada var*/
			exit;
		}
	}else{
		echo "error_2000";/*Vəzifə adı boşdur*/
		exit;
	} 
}
}
