...
document.getElementById("SeyfeAdi").innerHTML = "İşdən azad edilmə əmirləri";
...
function Yeni() {		
	...
	xhttp.open("POST", "IsdenAzadEtme/Yeni.php", true);
...
function SelectIkiAlaniSecildi(deyer) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "IsdenAzadEtme/Melumat_Telebi.php", true);
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp.send("ID=" + ID);
...
function	YeniFormKontrol(id){
	...
	var Isden_Cixarilma_Tarixi = document.getElementById("Isden_Cixarilma_Tarixi"); 
	var xitam_sebebleri_id = document.getElementById("xitam_sebebleri_id"); 
	var Isden_Cixarilma_Emir_No = document.getElementById("Isden_Cixarilma_Emir_No");
	if(ID.value === '') {
	...
	if(Isden_Cixarilma_Tarixi.value === '') {
		error(Isden_Cixarilma_Tarixi);
		return;
	}
	...
	if(Isden_Cixarilma_Emir_No.value === '') {
		error(Isden_Cixarilma_Emir_No);
		return;
	}
...
function YeniForm(){
	...
	var deyer = {
		ID:ID.value,
		Isden_Cixarilma_Tarixi:Isden_Cixarilma_Tarixi.value,
		xitam_sebebleri_id:xitam_sebebleri_id.value,
		Isden_Cixarilma_Emir_No:Isden_Cixarilma_Emir_No.value
	};
	var gonderilen=JSON.stringify(deyer);
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "IsdenAzadEtme/Yeni_Islemleri.php", true);
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function Sil_Tesdiq(id) {
	document.getElementById("yuklemealanikapsayici").style.display = "block";
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "IsdenAzadEtme/Sil.php", true);
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp.send("Deyer=" + id);
	xhttp.onreadystatechange = function (deyer) {
		if (this.readyState == 4 && this.status == 200) {
