...
if (isset($_POST['yeni'])) {?>
	<div class="row">						
		<form class="row p-2 ">		
			...
			<div class="col-2">
				<label for="Isden_Cixarilma_Tarixi" class="form-label">Tarixi<span class="KirmiziYazi">*</span></label>
				<input type="text" class="form-control pickmeup_1 tarix" autocomplete="off" onchange="TarixAlaniYazildi(this.id)" oninput="TarixAlaniYazildi(this.id)" onfocusout="TarixAlaniYazildi(this.id)" placeholder="<?php echo $TekTarix ?>" maxlength="10" id="Isden_Cixarilma_Tarixi"  title="">
			</div>


			<hr>
			<div class="col-8">
				<label for="xitam_sebebleri_id	" class="form-label">İşdən azad olunma səbəbi<span class="KirmiziYazi">*</span></label>
				<select id="xitam_sebebleri_id" required="required" class="form-select" onchange="SelectAlaniSecildi(this.id)" title="">
					<option disabled="disabled" value="" selected="selected" tabindex="7"></option>
					<?php 
					$Xitam_Sebeb_Sor=$db->prepare("SELECT * FROM  xitam_sebebleri order by xitam_sebebleri_id ASC ");
					$Xitam_Sebeb_Sor->execute();
						while ($Xitam_Sebeb_Cek=$Xitam_Sebeb_Sor->fetch(PDO::FETCH_ASSOC)) {						?>
							<option value="<?php echo $Xitam_Sebeb_Cek['xitam_sebebleri_id'] ?>" title="<?php echo $Xitam_Sebeb_Cek['xitam_sebebleri_ad'] ?>"><?php echo $Xitam_Sebeb_Cek['xitam_sebebleri_kisa_ad'] ?></option>
						<?php } ?>					
					</select>
				</div>

				<div class="col-2">
					<label for="Isden_Cixarilma_Emir_No" class="form-label">Əmrin nömrəsi<span class="KirmiziYazi">*</span></label>
					<input type="text" class="form-control" id="Isden_Cixarilma_Emir_No" oninput="MetinAlaniYazildi(this.id)" title="" onfocusout="MetinAlaniYazildi(this.id),SagVeSolBosluklariSIl(this.id)" maxlength = "150" tabindex="10" >
				</div>
				<hr>
