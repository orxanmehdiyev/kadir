			...
			while ($Idare_Cek=$Idare_Sor->fetch(PDO::FETCH_ASSOC)) {
				...
				while ($Sobe_Cek=$Sobe_Sor->fetch(PDO::FETCH_ASSOC)) {	
					...
					while ($Vezife_Cek=$Vezife_Sor->fetch(PDO::FETCH_ASSOC)) {								
						...
						$Atest_Sor=$db->prepare("SELECT * FROM  attestasiya_emri where ID=:ID and Attestasiya_Tarix_Novbeti<=:Attestasiya_Tarix_Novbeti order by Attestasiya_Tarix DESC ");
						$Atest_Sor->execute(array(
							'ID'=>$Cek['ID'],
							'Attestasiya_Tarix_Novbeti'=>date("Y-m-d",strtotime($_POST['Deyer']))
						));
						$Atest_Say=$Atest_Sor->rowCount();
						if ($Atest_Say) {
							$Atest_Cek=$Atest_Sor->fetch(PDO::FETCH_ASSOC);

							$Atest_Son=$db->prepare("SELECT * FROM  attestasiya_emri where ID=:ID  order by Attestasiya_Tarix DESC ");
							$Atest_Son->execute(array(
								'ID'=>$Cek['ID']
							));
							$Atest_SonCek=$Atest_Son->fetch(PDO::FETCH_ASSOC);

							$novbetison=$Atest_SonCek['Attestasiya_Tarix_Novbeti'];
							$faktiki=$Atest_Cek['Attestasiya_Tarix_Novbeti'];
							if ($novbetison<=$faktiki) {
							...
							$Atest_Sayimn=$db->prepare("SELECT * FROM  attestasiya_emri where ID=:ID");
							$Atest_Sayimn->execute(array(
								'ID'=>$Cek['ID']
							));			
							$Atest_Sayimnsay=$Atest_Sayimn->rowCount();	
							if ($Atest_Sayimnsay==0) {
								$yenigiren=	Traix_Uzerine_Gel($Cek['Ise_Qebul_Tarixi'], 1, "year");
								if ($yenigiren <= date("Y-m-d",strtotime($_POST['Deyer']))) {?>
									<tr class="vertikalmidle">
										<td class="textaligncenter"><?php echo $Vezife_Cek['User_Id']>0 ? $Vezife_Cek['User_Id']:"";?></td>
										<?php if ($Vezife_Cek['User_Id']>0) {?>
											<td style="padding: 0; cursor: pointer;" ><a href="<?php echo "personal-".$Cek['Seo_Url']."-".$Cek['ID'] ?>" target="_blank"><?php echo $RutbeSekli ?></a></td>
