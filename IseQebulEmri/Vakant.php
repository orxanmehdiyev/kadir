...
if ( $Vezife_Say>0) {
	if ($Stat_Muqavile==0) {
		...
		<div class="col-5">
			<label for="User_Islediyi_Idare" class="form-label">İdarə<span class="KirmiziYazi">*</span></label>
			<select class="form-select" required="required"  id="User_Islediyi_Idare" onchange="StatIdareSobeTelebEt(this.id)" tabindex="12" title="" >
				<option ></option>
				<?php
	...
	<?php }else{?>
		...
		<div class="col-5">
			<label for="User_Islediyi_Idare" class="form-label">İdarə<span class="KirmiziYazi">*</span></label>
			<select class="form-select" required="required"  id="User_Islediyi_Idare" onchange="MuqavileIdareSobeTelebEt(this.id)" tabindex="12" title="" >
				<option ></option>
				<?php
				$Idare_Sorr=$db->prepare("SELECT DISTINCT Idare_Id FROM vezife where NOT EXISTS (SELECT * FROM ise_qebul_emri WHERE  ise_qebul_emri.User_Vezife = vezife.Vezife_Id and ise_qebul_emri.Ise_Qebul_Emri_Stausu=:yeniemir ) and Stat_Muqavile=:Stat_Muqavile and  User_Id=:User_Id and Durum=:Durum");
				$Idare_Sorr->execute(array(
